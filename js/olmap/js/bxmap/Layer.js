...
bxmap.layer.Defaults.prototype.createPointABCDLayer = function(){
    ...
    var layer = new bxmap.layer.Vector({
		source: source,
        property: "symbolValue",
        context: function(feature){
            return feature["symbolValue"];
        }
    });
    //关联原始要素
 ...
 * 可符号化的矢量图层。
 * 说明：
 *   1. registerDetectingFeature/unregisterDetectingFeature/select/highlight/unhighlight方法需要依赖symbolizer，
 * 且符号化计算器symbolizer需要设置ruleName为selected和unselected，symbolizer的定义如示例1。
 *   2.监听事件 beforeselect | afterselect 由select方法触发；
 * 监听事件 over | out，只有registerDetectingFeature注册后，鼠标移动到|移除要素时触发。
 * @extends {ol.layer.Vector}
 * @constructor
 * @param options 与{ol.layer.Vector}参数一致，请参考{ol.layer.Vector}
 * @param options.symbolizer {bxmap.symbol.Symbolizer} 可选。符号化计算器
 * @param options.property {String} 可选。用于标识符号化要素值的字段，可以为feature字段名或属性
 * @param options.context {function(feature)} 可选。用于返回要素携带的值。例如返回feature.get("type")
 *
 * @fires beforeselect | afterselect | over | out
 * @example <caption> 示例1-定义可检测要素的符号化计算器 </caption>
 * var symbolizer = new bxmap.symbol.UniqueValueSymbolizer();
 * //ruleName内置为default/selected/unselected，否则检测时要素样式会设置为空，无法显示在地图上
 * symbolizer.addRule({ruleName: "default", styles: styles["Mariculture_Clickable_Selected"]});
 * symbolizer.addRule({ruleName: "selected", styles: styles["Mariculture_Clickable_Selected"]});
 * symbolizer.addRule({ruleName: "unselected", styles: styles["Mariculture_Clickable_Selected"]});
 * layer.setSymbolizer(symbolizer);
 */
bxmap.layer.Vector = function(options) {
	 ...
	 * @description 符号化样式规则
	 */
	this.symbolizer = opts.symbolizer;
     ...
     * @description 用于获取标识值
     */
    this._contextFunc = opts.context;
...
bxmap.layer.Vector.prototype._handleAddFeature = function(evt){
	...
	if(this.symbolizer && feature){
        var value = this._contextFunc(feature);
        this.symbolizer.evaluate(feature, value);
	}
}
 ...
 * @param {String | null} ruleName 规则名称，对应名称的规则为{key1: ol.style, key2: ol.style}
 * @param {String|undefined} value feature符号化使用的值
 * @returns {String}
 */
bxmap.layer.Vector.prototype.setFeaturesSymbolRuleName = function(features, ruleName, value){
	...
	for(var i = 0; i < features.length; i++){
		...
		if(ruleName == null){
			...
			symbolizer.evaluate(feature, value);
		}
	}
}
 ...
 */
bxmap.layer.Vector.prototype.registerDetectingFeature = function(bmap){
    var _this = this;
 ...
 * @param {bxmap.Map} bmap 地图
 */
bxmap.layer.Vector.prototype.unregisterDetectingFeature = function(bmap){
    var _this = this;
    bmap.setFeatureDetectingActive({
        type: "pointermove",
        target: this,
 ...
 */
bxmap.layer.Vector.prototype.select = function(feature){
    //取消lastFeature选中状态
    var lastFeature = this._lastSelected;
    this.removeFeatureClickedState(lastFeature);
    if(lastFeature){
    ...
    if(feature == null){
        ...
        //设置当前要素选中状态
        this.addFeatureClickedState(feature);
 ...
 */
bxmap.layer.Vector.prototype.highlight = function(features){
    if(features == null){
        var tempFeatures = this.getSource().getFeatures();
        this.unhighlight(tempFeatures);
    }else{
        //this.setFeaturesSymbolByValue(features, true);
		for(var i = 0 ; i < features.length; i++) {
			var feature = features[i];
            var val = this._contextFunc(feature);
            this.setFeaturesSymbolRuleName([feature], "selected", val);
        }
    }
}
 ...
 */
bxmap.layer.Vector.prototype.unhighlight = function(features) {
    if (features == null) return;
    //feature符号化使用的值
	for(var i = 0; i < features.length; i++) {
		var feature = features[i];
        var val = this._contextFunc(feature);
        this.setFeaturesSymbolRuleName([feature], "unselected", val);
    }
}

/**
 * @description 添加要素click检测状态。
 * 添加该状态后，由highlight方法高亮的要素在鼠标移除要素范围时仍然保持高亮状态；
 * 若没有该状态，由highlight方法高亮的要素在鼠标移除要素范围时将清除高亮效果；
 * @param {ol.Feature} feature
 */
bxmap.layer.Vector.prototype.addFeatureClickedState = function(feature){
    if(feature){
 ...
 * @param {ol.Feature} feature
 */
bxmap.layer.Vector.prototype.removeFeatureClickedState = function(feature){
    if(feature){
        var events = bxmap.common.getFeatureDetectedParams(feature);
        delete events["click"];
    }
 ...
 */
bxmap.layer.Vector.prototype._handleFeatureState = function(feature, lastFeature){
    //设置上一个要素非选中状态
    if(lastFeature){
 ...
 * @constructor
 * @param {Object} options
 * @param options.source {ol.source.Vector} 可选。默认new ol.source.Vector()
 * @param options.style {ol.style.Style | undefined} 原始要素默认显示样式，未选中样式
 * @param options.property {String} 用于标识要素值的字段，可以为feature字段名或属性。property与context用于为原始要素额外添加ABCD位置点渲染符号
 * @param options.context {function(feature)} 必需。用于返回要素携带的值。例如返回feature.get("type")
 * @fires beforeselect | afterselect | over | out
 */
bxmap.layer.AbcdSymbolicLayer = function(options) {
	 ...
	 * @description 未选中要素样式
	 */
	this._unselectedStyle = opt_options.style === undefined ? bxmap.common.getDefaultVectorLayerStyle() : opt_options.style;
	/**
	 * @private
    ...
    /**
	 * @readonly
	 * @description 由于该类会新增额外的要素，如需获取原始真正的要素，需使用该属性
     * @type {*|ol.source.Vector}
     */
    this.realFeatures = [];

	//存储要素
	opt_options.source = opt_options.source || new ol.source.Vector();
	bxmap.layer.Vector.call(this, opt_options);
...
bxmap.layer.AbcdSymbolicLayer.prototype._handleAddFeature = function(evt){
	...
	//真实要素集
    this.realFeatures.push(feature);
	...
	if(this._contextFunc && this._property){
	...
	if(this.symbolizer){
		var value = this._contextFunc ? this._contextFunc(pointFeature) : undefined;
		this.symbolizer.evaluate(pointFeature, value);
	}
}
...
bxmap.layer.AbcdSymbolicLayer.prototype._toPointGeometry = function(geometry){
	...
	switch(geometry.getType()){
		...
		case "Point":
			pointGeom = geometry.clone();
			break;
		case "MultiLineString":
			geometry = geometry.getLineString(0);
		case "LineString":
			pointGeom = new ol.geom.Point(geometry.getCoordinateAt(0.5));
			break;
		case "MultiPolygon":
...
bxmap.layer.AbcdSymbolicLayer.prototype.unhighlight = function(features){
	...
	this.setFeaturesSymbolByValue(features, false);
	// var feature, uid;
	// for(var i = 0; i < features.length; i++){
	// 	feature = features[i];
	// 	uid = bxmap.common.getCounterUid(feature).toString();
...
/**
 * 根据feature符号化使用的值设置样式
 * @param {Array<ol.Feature>} features 要素数组
 * @param {String | null} ruleName 规则名称，对应名称的规则为{key1: ol.style, key2: ol.style}
 * @param {String| undefined} value feature符号化使用的值
 */
bxmap.layer.AbcdSymbolicLayer.prototype.setFeaturesSymbolByValue = function(features, selected, value){
	var ruleName = selected ? "selected" : "unselected"; //规则名称
	var featureStyle = ruleName == "selected" ? this._selectedStyle : this._unselectedStyle;
    ...
    for(var i = 0; i < features.length; i++){
        ...
        for(var ii = 0; ii < tempFeatrues.length; ii++){
            ...
            if(this._existsExtraFlags(temp)){
            	//feature符号化使用的值
            	if(!value && this._contextFunc){
                    val = this._contextFunc(feature) ;
				}
                this.setFeaturesSymbolRuleName([temp], ruleName, val);
            }else if(featureStyle){
                temp.setStyle(featureStyle);
            }
        }
    }
}
 ...
 * @param {ol.Feature} feature
 */
bxmap.layer.AbcdSymbolicLayer.prototype.addFeatureClickedState = function(feature){
	if(feature){
		var uid = bxmap.common.getCounterUid(feature).toString();
		var features = this._featureCache[uid];
		var temp;
 ...
 * @param {ol.Feature} feature
 */
bxmap.layer.AbcdSymbolicLayer.prototype.removeFeatureClickedState = function(feature){
	if(feature){
		var uid = bxmap.common.getCounterUid(feature).toString();
		var features = this._featureCache[uid];
		var temp;

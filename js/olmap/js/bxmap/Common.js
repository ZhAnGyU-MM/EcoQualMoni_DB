...
;(function (root, factory) {
    ...
    var traceStyles = null;

    /**
     * 获取默认的轨迹样式
     */
    function getDefaultTraceStyles() {
        if(!traceStyles){
            traceStyles = {};
            traceStyles["startPoint"] = new ol.style.Style({
                image: new ol.style.Icon({
                    size: [26, 32],
            ...
            traceStyles["endPoint"] = new ol.style.Style({
                image: new ol.style.Icon({
                    size: [26, 32],
    ...
    /**
     * @description 读取wkt点数据
     * @function
     * @memberof bxmap.common
     * @static
     * @param pointWKT {String} "POINT(117.23,23.45)"
     * @returns [x,y]
     */
    function readCoordinateFromWKT(pointWKT) {
        var output = null;
        if (!pointWKT) return output;
        var pattern = /(-?\d+)(\.\d+)?/g;
        var xy = pointWKT.match(pattern);
    ...
    return {
        ...
        getWaterDepthLabelStyles: getWaterDepthLabelStyles, //水深点样式
        getDefaultTraceStyles: getDefaultTraceStyles, //轨迹样式
        offsetCoordinate: offsetCoordinate,//坐标偏移dx,dx像素
        readCoordinateFromWKT: readCoordinateFromWKT //读取wkt点数据
    }
});
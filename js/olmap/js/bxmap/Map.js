...
bxmap.Map.prototype.initializeMap = function(target, view) {
	...
	var map = this.map = new ol.CanvasMap({
...
/**
 * @description  设置中心点
 * @param center {Array<Number>} [x,y] 中心点坐标
 * @param offset {Array<Number>} 可选。[px,py] 屏幕坐标系x/y方向分别偏移多少像素，X正值向左偏移，Y正值向下偏移
 */
bxmap.Map.prototype.setCenter = function (center, offset) {
    var map = this.getMap();
 ...
 * @param offset {Array<Number>} 可选。[px,py] 屏幕坐标系x/y方向分别偏移多少像素，X正值向左偏移，Y正值向下偏移
 */
bxmap.Map.prototype.zoomToExtent = function(extent, offset){
	//判断范围是否有效
	function isEmpty(ex) {
		var empty = false;
		if(ex == null || ex.length != 4){
			empty = true;
		}else if(Math.abs(ex[0]) == Infinity || Math.abs(ex[1]) == Infinity
            || Math.abs(ex[2]) == Infinity || Math.abs(ex[3]) == Infinity){
			empty = true;
		}
        return empty;
    }
	...
	var zoom = view.getZoomForResolution(resolution) - 2;
    zoom = Math.min(zoom, this.ZoomMaxLevel);

    view.setZoom(zoom);
    //设置偏移
    var center = ol.extent.getCenter(extent);
    this.setCenter(center, offset);
}
 ...
 * @param offset {Array<Number>} 可选。[px,py] 屏幕坐标系x/y方向分别偏移多少像素，X正值向左偏移，Y正值向下偏移
 * @param zoom {Number} 可选。指定缩放级别
 */
bxmap.Map.prototype.zoomToFeature = function(feature, offset, zoom){
	...
	//不指定zoom时分别处理
        ...
        this.zoomToExtent(geometry.getExtent(), offset);
...
//  * @param zoom {Number} 可选。指定缩放级别
//  */
// bxmap.Map.prototype.zoomToLayer = function(layer, zoom){
...
//         var et = extent;
//         view.setZoom(zoom);
//         view.setCenter([(et[0] + et[2])/2 , (et[1] + et[3])/2]);
//     }else{
//         view.fit(extent, {
//             size: map.getSize()
//         });
 ...
 * @param color {String|undefined} 默认为红色闪烁，示例'rgba(237,28,36,0.4)'|'rgb(237,28,36)'|'#ed1c24'
 * @constructor
 */
bxmap.Map.prototype.FlashFeature = function (feature, color) {
	if(feature == null) return;
	//定位至要素
    this.zoomToFeature(feature);

    //获取默认闪烁样式
    function getFlashStyles(c) {
        var styles = {};
        var white = "#ffffff";
        var red = c ? c : "#ed1c24";
        var width = 3;
        ...
        styles["Polygon"] = new ol.style.Style({
            stroke: styles["LineString"].getStroke(),
            fill: new ol.style.Fill({
    ...
    var styles = getFlashStyles(color);
    var flashStyle = styles[geoType];
    ...
    function animate(event) {
        ...
        var style = index % 2 == 0 ? flashStyle : defaultStyle;
 ...
 * @param inswitcher - {Boolean} 是否动态添加到图层管理器中
 * @returns {Boolean} 添加成功返回true,否则返回false
 */
bxmap.Map.prototype.addIndexLayer = function(layer, inswitcher){
	...
	//需要动态添加到图层管理器
	if(inswitcher){
		if(!layer.switcherSetting){
            layer.switcherSetting = {
                "id": index,
				"pid": bxmap.LAYER_SWITCHER_ROOT,
                "inswitcher": true,
				"visible":true,
				"title": "内存图层",
				"tip": "",
                "bxmap.LAYER_SWITCHER_CATEGORY": bxmap.LAYER_SWITCHER_CATEGORY_LAYER
            };
		}
		this.getBaseLayerGroup().getLayers().push(layer);
	}
	//指定对应的内存图层
	else if(layer["memory"]){
		var reader = this.getMapConfig();
		var group = reader.getShareLayer(layer["memory"]);
		if(group){
 ...
 * @param evt.pointermoveLayer {ol.layer.Vector} 当前pointermove监测到的要素所在的图层
 ...
 * @param options.target {Object} (必需)需要使用引用类对象，target相同时，仅最新定义的有效。设置指定对象开启/关闭检测要素，
 * 用于标记map.on(type, this._handleFeatureDetectingEvent, target)中target注册事件的对象，
...
bxmap.Map.prototype.setFeatureDetectingActive = function(options){
	...
	var tempVars = target._tempVars = target._tempVars || {};
	//检测临时变量
...
bxmap.Map.prototype._handleFeatureDetectingEvent = function(evt){
	...
	map.forEachFeatureAtPixel(evt.pixel, function(feature, layer){
		...
		if(evt.type == "click"){
            ...
            options["pointermoveLayer"] = layer;
		}
	}, {
	...
	switch(evt.type){
		case "pointermove":
			//currentFeature或lastFeature存在且 lastFeature不能等于currentFeature时才可以执行
			var canHandle = (currentFeature || lastFeature) && (currentFeature !== lastFeature);
			if(!canHandle) return;
			bmap._handlePointerMoveDetectedFeature(currentFeature, lastFeature, target, evt);
			break;
		case "click":
			bmap._handleClickDetectedFeature(currentFeature, lastFeature, target, evt);
			break;
		default:
			break;
	}
 ...
 */
bxmap.Map.prototype._handlePointerMoveDetectedFeature = function(currentFeature, lastFeature, target, evt){
	var type = "pointermove";
	var featureDetectingVars = target._tempVars["featureDetectingVars"];
	var opt_key = type + "_featureDetectingParams";
	var options = featureDetectingVars[opt_key];
	...
	if(options.callback){
		...
		options.callback({
			...
			bmap: this,
            pointermoveLayer: options["pointermoveLayer"]
		});
	}
 ...
 * @param currentFeature
 */
bxmap.Map.prototype._handleClickDetectedFeature = function(currentFeature, lastFeature, target, evt){
	var type = "click";
	var featureDetectingVars = target._tempVars["featureDetectingVars"];
	var opt_key = type + "_featureDetectingParams";
	var options = featureDetectingVars[opt_key];

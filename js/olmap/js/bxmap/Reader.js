...
bxmap.reader.MapConfigReader.prototype._readSwitcherLayersByGroupId = function(group_id, switcherLayers){
	...
	for(var i=0,length = switcherLayers.length; i<length;i++){
		...
		if(isLayerGroup){
            ...
            layer.switcherSetting[setting.category] = catagory;
            ...
            layer.switcherSetting = {};
            layer.switcherSetting[setting.category] = bxmap.LAYER_SWITCHER_CATEGORY_LAYER;
			//layer.set(setting.category,bxmap.LAYER_SWITCHER_CATEGORY_LAYER);
		}
		if(layer){
			//图层控制需要的参数设置
            layer.switcherSetting = layer.switcherSetting || {};
            layer.switcherSetting[setting.inswitcher] = displayInSwitcher;//是否显示在图层控制器上
            layer.switcherSetting[setting.owned_base_group] = group_id;//所属图层组
            layer.switcherSetting[setting.id] = layerObj[setting.id];//图层id
            layer.switcherSetting[setting.pid] = layerObj[setting.pid];//父图层id
            layer.switcherSetting[setting.title] = layerObj[setting.title];//显示名称
            layer.switcherSetting[setting.tip] = layerObj[setting.tip] ? layerObj[setting.tip]:"";//提示信息
            layer.switcherSetting[setting.visible] = layerObj[setting.visible] == null ? true:layerObj[setting.visible];//提示信息
            layer.setVisible(layer.switcherSetting[setting.visible]); //可见
